...
bool Selection::ApplyWCSelection(EventContainer &_evt, const BDTTool &_bdt, Utility::FileTypeEnums type, Utility::RunPeriodEnums runPeriod) {
...
bool Selection::ApplyCutBasedSelection(EventContainer &_evt, Utility::FileTypeEnums type, Utility::RunPeriodEnums runPeriod) {
	...
	// variable missing in these samples
	...
	//if (!_evt.slice_orig_pass_id) return false;	    // slice original pass ID

	// generic neutrino selection
	if (_evt.wc_numu_cc_flag < 0) return false;		// WC generic neutrino selection
	
	// Fiducial volume cut
	// require both Pandora and WC neutrino vertex in fiducial volume 
	if(!ApplyVertexFVCut(_evt.reco_nu_vtx_sce_x, _evt.reco_nu_vtx_sce_y, _evt.reco_nu_vtx_sce_z)) return false;
	// is WC vertex only filled?
	if(!ApplyVertexFVCut(_evt.wc_reco_nuvtxX, _evt.wc_reco_nuvtxY, _evt.wc_reco_nuvtxZ)) return false;

	//if (_evt.vertex_discrepancy > 10) return false;

	// count number of primary tracks
	int n_primary_tracks = 0;
	int n_primary_track_exiting = 0;

	for (unsigned int idx = 0; idx < _evt.trk_sce_start_x_v->size(); idx++) {
		
		// check if track is primary
		if (_evt.pfp_generation_v->at(idx) != 2) continue;
		
		// track score
		if (_evt.trk_score_v->at(idx) < 0.5) continue;		

		// check if track is exiting
		if (!_utility.isExiting(_evt.trk_sce_end_x_v->at(idx), 
										 _evt.trk_sce_end_y_v->at(idx),
										 _evt.trk_sce_end_z_v->at(idx))) {
			n_primary_track_exiting++;
		}
	...
	_evt.n_primary_tracks = n_primary_tracks;
	_evt.n_primary_track_exiting = n_primary_track_exiting;

	// count number primary showers
	int n_primary_showers = 0;
	for (unsigned int idx = 0; idx < _evt.shr_start_x_v->size(); idx++) {
		// check if shower is primary
		if (_evt.pfp_generation_v->at(idx) != 2) continue;
		
		// shower score
		if (_evt.trk_score_v->at(idx) >= 0.5) continue;

		// increment primary shower count
		n_primary_showers++;
	}

	_evt.n_primary_showers = n_primary_showers;

	// require match signal definition
	// signal canidate cut
	// at least 1 primary track, 0 primary showers 
	if(!ApplySignalCanidateCut(_evt.n_primary_showers, _evt.n_primary_tracks)) return false;

	// muon veto

	// no exiting primary tracks
	if (_evt.n_primary_track_exiting > 0) return false;

	// leading track much be at least 10 cm long and no more than 150 cm long
	if (_evt.trk_len < 5) return false; // 10 cm
	if (_evt.trk_len > 150) return false; // 150 cm

	/*
    // count number of non-proton tracks
	int n_non_proton_tracks = 0;
	// loop through primary tracks
	for (unsigned int i_trk = 0; i_trk < _evt.trk_sce_start_x_v->size(); i_trk++) {			

		// check if track is primary
		if (_evt.pfp_generation_v->at(i_trk) != 2) continue;

		// hits on all planes
		//if (!ApplyHitsOnAllPlanesCut(_evt.pfnplanehits_U->at(i_trk), _evt.pfnplanehits_V->at(i_trk), _evt.pfnplanehits_Y->at(i_trk))) continue;
		
		// track score
		if (_evt.trk_score_v->at(i_trk) < 0.5) continue;

		// vertex distance
		//if (_evt.trk_distance_v->at(i_trk) > 4) continue; // 4cm

		// track length
		//if (_evt.trk_len_v->at(i_trk) < 10) continue; // 10cm
		
		// LLR PID score
		if (_evt.trk_llr_pid_score_v->at(i_trk) <= 0 ) continue;

		// NuGraph HIP fraction
		if (_evt.pfng2hipfrac->at(i_trk) >= 0.5) continue; // 50% HIP fraction
	
		n_non_proton_tracks++;
	}

	_evt.n_non_proton_tracks = n_non_proton_tracks;

	// require 1 and only 1 non-proton track
	//if (n_non_proton_tracks != 1) return false;

	// count pion candidate tracks, muon rejection
	int n_pion_candidate_tracks = 0;
	// loop through primary tracks
	for (unsigned int i_trk = 0; i_trk < _evt.trk_sce_start_x_v->size(); i_trk++) {			
		...
		// track length
		if (_evt.trk_len_v->at(i_trk) < 10) continue; // 10cm
		if (_evt.trk_len_v->at(i_trk) > 150) continue; // 200cm
		...
		n_pion_candidate_tracks++;
	}

	_evt.n_pion_candidate_tracks = n_pion_candidate_tracks;

	//if (n_pion_candidate_tracks == 1) return false; // require at least 1 pion candidate track
	*/

	// --- WC SELECTION ---
	

	// check if event is in FV
	//if (!_utility.inFV(_evt.wc_reco_nuvtxX, _evt.wc_reco_nuvtxY, _evt.wc_reco_nuvtxZ)) return false; // WC vertex in FV	
	...
	//std::cout << "Number of WC tracks: " << _evt.wc_reco_Ntrack << std::endl;

	for (unsigned int wc_idx = 0; wc_idx < _evt.wc_reco_Ntrack; wc_idx++) {

		//std::cout << "WC track index: " << wc_idx << std::endl;
		...
		//std::cout << "WC track length: " << wc_track_length << std::endl;
	...
	//std::cout << "WC primary track vertex separation: " << _evt.wc_primaryTrackVertexSeparation << std::endl;

	// at least one WC track
	if(_evt.wc_primaryTrackIndex == -1) return false;

	// require primary track within 4cm of vertex
	if(_evt.wc_primaryTrackVertexSeparation > 4) return false; // primary track within 4cm of vertex

	// track length
	if (_evt.wc_primaryTrackLength < 5) return false; // WC primary track length > 10 cm
	if (_evt.wc_primaryTrackLength > 150) return false; // WC primary track length < 150 cm
	...
	for (unsigned int wc_idx = 0; wc_idx < _evt.wc_reco_Ntrack; wc_idx++) {
		...
		// not interested in particles disconected from vertex, potentially miss-IDed pion daughters
		...
		if (wc_TrackVertexSeparation > 5) continue; // 10cm, matching minimum track length

		//std::cout << "WC PDG: " << _evt.wc_reco_pdg[wc_idx] << std::endl;
		//std::cout << "Lantern LArPID: " << _evt.wc_reco_larpid_pdg[wc_idx] << std::endl;
	...
	// store candidate index
	_evt.wc_pion_candidate_index = wc_pion_candidate_indices[0];
	...
	for (unsigned int wc_idx = 0; wc_idx < _evt.wc_reco_Ntrack; wc_idx++) {
		...
		// increment daughter total energy
		wc_pion_candidate_daughters_total_energy += _evt.wc_reco_startMomentum[wc_idx][3]; // E, is this visible energy?
	}
	...
	//std::cout << "Number of blips: " << _evt.nblips_saved << std::endl;
	...
	for (unsigned int i_blip = 0; i_blip < _evt.nblips_saved; i_blip++) {
		...
		//std::cout << "Blip " << i_blip << ": (" << blip_x << ", " << blip_y << ", " << blip_z << "), Distance from end: " << distance_from_end << " cm" << std::endl;
	...
	// LArPID pion score requirement
	//if (_evt.wc_reco_larpid_pidScore_pi[_evt.wc_pion_candidate_index] < -0.5) return false;
		
	// LArPID muon score requirement, reject highly muon-like tracks
	//if (_evt.wc_reco_larpid_pidScore_mu[_evt.wc_pion_candidate_index] > -1.5) return false; // reject muon-like tracks

	// LArPID proton score requirement, reject highly proton-like tracks
	//if (_evt.wc_reco_larpid_pidScore_pr[_evt.wc_pion_candidate_index] > -3) return false; // reject proton-like tracks



	/*
	std::cout << "WC Primary Track Index: " << _evt.wc_primaryTrackIndex
			  << ", Length: " << _evt.wc_primaryTrackLength << " cm"
			  << ", Vertex Separation: " << _evt.wc_primaryTrackVertexSeparation << " cm"
			  << ", PDG: " << _evt.wc_reco_pdg[_evt.wc_primaryTrackIndex]			 
			  << ", LArPID Classified: " << _evt.wc_reco_larpid_classified[_evt.wc_primaryTrackIndex]
			  << ", LArPID Primary Score: " << _evt.wc_reco_larpid_procScore_prim[_evt.wc_primaryTrackIndex]
			  << ", LArPID Score (el): " << _evt.wc_reco_larpid_pidScore_el[_evt.wc_primaryTrackIndex]
			  << ", LArPID Score (ph): " << _evt.wc_reco_larpid_pidScore_ph[_evt.wc_primaryTrackIndex]
			  << ", LArPID Score (mu): " << _evt.wc_reco_larpid_pidScore_mu[_evt.wc_primaryTrackIndex]
			  << ", LArPID Score (pi): " << _evt.wc_reco_larpid_pidScore_pi[_evt.wc_primaryTrackIndex]
			  << ", LArPID Score (pr): " << _evt.wc_reco_larpid_pidScore_pr[_evt.wc_primaryTrackIndex]
			  << std::endl;
	*/
	/*
	// require match signal definition
	// signal canidate cut
	// At least 1 track, 0 showers 
	if(!ApplySignalCanidateCut(_evt.n_showers, _evt.n_tracks)) return false;
	
	// at least one WC track
	//if(_evt.wc_primaryTrackIndex == -1) return false;

	// require Pandora primary track daughter of neutrino
	if(_evt.trk_pfpgeneration != 2) return false; // primary Pandora track is a daughter of the neutrino

	// require primary track contained
	bool primaryIsExiting = _utility.isExiting(_evt.trk_sce_end_x, _evt.trk_sce_end_y, _evt.trk_sce_end_z);
	//bool secondaryIsExiting = _utility.isExiting(_evt.trk2_sce_end_x, _evt.trk2_sce_end_y, _evt.trk2_sce_end_z);
	if (primaryIsExiting) return false;
	//if (_evt.trk2_id != 0 && secondaryIsExiting) return false;

	// require primary track within 4cm of vertex
	if(_evt.trk_distance > 4) return false; // primary track within 4cm of vertex

	// track length
	if (_evt.trk_len < 10) return false; // Pandora primary track length > 10 cm
	if (_evt.trk_len > 150) return false; // Pandora primary track length < 150 cm

	// reject events with long secondary tracks
	if (_evt.trk2_len > 50) return false; // Pandora secondary track length

	// require primary track muon-like LLRPID
	if (_evt.trk_llr_pid_score < 0.2) return false; // Pandora primary track LLRPID muon-like score > 0.2

	// require primary track to be classified by LArPID
	//if (_evt.wc_reco_larpid_classified[_evt.wc_primaryTrackIndex] == 0) return false; // WC primary track classified by LArPID
	
	// track length
	//if (_evt.wc_primaryTrackLength < 10) return false; // WC primary track length > 10 cm
	//if (_evt.wc_primaryTrackLength > 150) return false; // WC primary track length < 150 cm
	...
	*/
	...
	// length requirement on primary track
	if (_evt.trk_length < 10) return false; // 10 cm minimum length
	if (_evt.trk_length > 150) return false; // 150 cm maximum length
	*/
	/*
